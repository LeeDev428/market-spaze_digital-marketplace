...
export default function ManageVendors({ vendors, filters }: Props) {
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {vendors.data?.map((vendor: User) => (
                                    <tr key={vendor.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="flex-shrink-0 h-12 w-12">
                                                    <div className="h-12 w-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-white font-medium text-lg">
                                            ...
                                            <div className="space-y-2">
                                                {vendor.vendorStore ? (
                                                    <>
                                                        ...
                                                        {vendor.vendorStore.address && (
                                                            <div className="flex items-center">
                                                                <MapPin size={12} className="mr-1 text-gray-400" />
                                                                <span className="text-xs truncate max-w-xs">{vendor.vendorStore.address}</span>
                                                            </div>
                                                        )}
                                                    </>
                                                ) : (
                                                    <span className="text-xs text-gray-500">No store setup</span>
                                                )}
                                                <div className="flex items-center space-x-4 text-xs text-gray-500">
                                                    <div className="flex items-center">
                                                        <Package size={12} className="mr-1 text-gray-400" />
                                                        <span>{vendor.vendor_orders_count || 0} orders</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {getStatusBadge(vendor.is_activated)}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center justify-end space-x-2">
                                                <Link
                                                    href={`/admin/users/vendor/${vendor.id}`}
                                                ...
                                                <button
                                                    onClick={() => toggleUserStatus(vendor.id, vendor.is_activated)}
                                                    className={`${
                                                        vendor.is_activated 
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 sm:px-6">
                        <div className="flex justify-between flex-1 sm:hidden">
                            <button
                                onClick={() => vendors.prev_page_url && handlePagination(vendors.prev_page_url)}
                                disabled={!vendors.prev_page_url || loading}
                                className="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Previous
                            </button>
                            <button
                                onClick={() => vendors.next_page_url && handlePagination(vendors.next_page_url)}
                                disabled={!vendors.next_page_url || loading}
                        ...
                        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <div>
                                <p className="text-sm text-gray-700 dark:text-gray-300">
                                    ...
                                    <span className="font-medium">{vendors.last_page}</span> for vendors
                                </p>
                            </div>
                            <div>
