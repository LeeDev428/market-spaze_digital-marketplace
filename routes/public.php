...
Route::get('/service-details/{id}', function ($id) {
    ...
    $serviceData = [
        ...
        'includes' => is_array($service->includes) ? $service->includes : (is_string($service->includes) ? json_decode($service->includes, true) ?? [] : []),
        'requirements' => is_array($service->requirements) ? $service->requirements : (is_string($service->requirements) ? json_decode($service->requirements, true) ?? [] : []),
